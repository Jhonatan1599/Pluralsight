...
{
    /* This interface is used to decorate test classes to indicate a test which has per-test-class fixture data, so there is no new methods to implement. An instance of the type we pass through, so our EmployeeServiceFixture, will be instantiated right before the first test is run, and it will potentially be cleaned up once the last test has been run
     But how do we gain access to the properties from that fixture then?
        We add a constructor to our test class, and that constructor accepts the fixture as parameter. XUnit will make sure that an instance of it is injected. If we then store the fixture on our class, we can access its properties via the variables we stored it in
     */
    ...
    {   
        ...
        public async Task CreateInternalEmployee_InternalEmployeeCreated_AttendedCoursesMustMatchObligatoryCourses_Async()
        {
            ...
            var internalEmployee = await _employeeServiceFixture
                .EmployeeService.CreateInternalEmployeeAsync(
                "Brooklyn", "Cannon");
